OC.L10N.register(
    "preferred_providers",
    {
    "User enabled and verification email sent!" : "Учётная запись активирована и отправлено эл. письмо подтверждения.",
    "Error while sending the verification email" : "Не удалось отправить эл. письмо подтверждения",
    "Resend verification email and enable" : "Повторно оправить эл. письмо подтверждения и активировать запись пользователя",
    "The token is invalid" : "Недействительный токен",
    "The token is expired, please contact your provider" : "Истёк срок действия токена, свяжитесь с вашим поставщиком услуг",
    "Password too long" : "Пароль очень длинный",
    "Unable to set the password. Contact your provider." : "Невозможно установить пароль, свяжитесь с вашим поставщиком услуг.",
    "One more step required" : "Еще один шаг",
    "It looks like you did not create your password yet." : "Похоже, вы еще не придумали ваш пароль.",
    "Did you encounter an issue during the sign-up process?" : "Вы столкнулись с трудностями в ходе регистрации?",
    "Access your account" : "Доступ к учётной записи",
    "Welcome to your %s account" : "Добро пожаловать в ваш %s аккаунт",
    "Your %s account %s is now verified!" : "Ваш %s аккаунт%s подтвержден!",
    "Start using %s" : "Начните использовать %s",
    "Install mobile or desktop client" : "Установить приложение для ПК или мобильных устройств",
    "Verify your %s account" : "Подтвердите ваш %s аккаунт",
    "Just one step left to complete your account setup." : "Последний шаг настройки учётной записи.",
    "Click here to verify your email address" : "Нажмите здесь, чтобы подтвердить вашу электронную почту",
    "Your %s account has been disabled" : "Ваша учётная запись %s была отключена",
    "Your %s account %s has been disabled because it was not verified in time." : "Вы не подтвердили свой %s аккаунт%s вовремя и он был отключён.",
    "Please contact your provider for further assistance." : "Обратитесь к своему провайдеру для получения дополнительной помощи.",
    "Simple signup" : "Простая регистрация",
    "A confirmation mail was sent to {email}. Make sure to confirm the account within 6 hours." : "Письмо с подтверждением было отправлено на {email}. Необходимо подтвердить в течение 6 часов.",
    "Please verify your email address" : "Пожалуйста, подтвердите ваш электронный адрес",
    "Preferred providers" : "Предпочитаемые поставщики услуги",
    "Preferred Providers" : "Предпочитаемые Поставщики Услуг",
    "Allow nextcloud to request user accounts" : "Разрешить nextcloud запрашивать аккаунты пользователя",
    "Registration handling app for Nextcloud partners only" : "Приложение для обработки регистрации только для партнеров Nextcloud",
    "Set your password" : "Задайте свой пароль",
    "Account name or email" : "Учетная запись или адрес эл. почты",
    "Password" : "Пароль",
    "Set password" : "Задайте пароль",
    "Log in" : "Войдите",
    "After this step, you will receive a mail to verify your account within 6 hours." : "По завершению этого шага в течении 6 часов будет направлено эл. письмо для подтверждения учётной записи.",
    "Preferred providers token" : "Токен предпочитаемых поставщиков услуги",
    "WARNING! This token is very important and must be handled carefully. You should only give it to a Nextcloud official!" : "ВНИМАНИЕ! Этот токен крайне важен, обращайтесь с осторожностью. Вы должны предоставить его представителю Nextcloud!",
    "Your provider token" : "Токен провайдера",
    "Reset your token" : "Сбросить токен",
    "Preferred providers groups" : "Группы предпочитаемых поставщиков услуги",
    "Select the groups to which each new account will be added." : "Выберите группы, в которые будет добавляться каждая новая учетная запись.",
    "Select the groups that will be assigned to each unconfirmed account. They will be deleted once the account has been verified." : "Выберите группы, которые будут назначены каждой неподтвержденной учетной записи. Учетная запись будет исключена из группы, как только будет подтверждена.",
    "Select the groups that will be assigned to each confirmed account." : "Выберите группы, которые будут назначены каждой подтвержденной учетной записи."
},
"nplurals=4; plural=(n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10<=4 && (n%100<12 || n%100>14) ? 1 : n%10==0 || (n%10>=5 && n%10<=9) || (n%100>=11 && n%100<=14)? 2 : 3);");
