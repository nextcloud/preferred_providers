OC.L10N.register(
    "preferred_providers",
    {
    "User enabled and verification email sent!" : "Používateľ povolený a overovací e-mail poslaný!",
    "Error while sending the verification email" : "Počas odosielania notifikačného e-mailu nastala chyba",
    "Resend verification email and enable" : "Pošli notifikačný e-mail znova a povol",
    "The token is invalid" : "Token je neplatný",
    "The token is expired, please contact your provider" : "Platnosť tokenu skončila, kontaktujte prosím svojho poskytovateľa",
    "Password too long" : "Heslo je príliš dlhé",
    "Unable to set the password. Contact your provider." : "Nie je možné nastaviť heslo. Obráťte sa na svojho poskytovateľa.",
    "One more step required" : "Je potrebný ešte jeden krok",
    "It looks like you did not create your password yet." : "Vyzerá to tak, že zatiaľ nemáte vytvorené heslo.",
    "Did you encounter an issue during the sign-up process?" : "Narazili ste na problém pri procese registrácie?",
    "Access your account" : "Pristúpte k svojmu účtu",
    "Welcome to your %s account" : "Vitajte vo svojom %s účte",
    "Your %s account %s is now verified!" : "Váš %s účet %s je teraz overený!",
    "Start using %s" : "Začať používať %s",
    "Install mobile or desktop client" : "Nainštalovať mobilného alebo desktopového klienta",
    "Verify your %s account" : "Overte svoj %s účet",
    "Just one step left to complete your account setup." : "Na dokončenie nastavenia účtu ostáva už len jeden krok.",
    "Click here to verify your email address" : "Kliknutím sem overíte svoju adresu e-mailu",
    "Your %s account has been disabled" : "Váš účet %s bol zakázaný",
    "Your %s account %s has been disabled because it was not verified in time." : "Váš %s účet %s bol zakázaný, pretože nebol overený v stanovenom časovom limite.",
    "Reactivate your account" : "Znova aktivovať konto",
    "Simple signup" : "Jednoduché prihlásenie",
    "A confirmation mail was sent to {email}. Make sure to confirm the account within 6 hours." : "Notifikačný e-mail bol odoslaný na {email}. Do 6 hodín musíte účet overiť.",
    "Please verify your email address" : "Prosím, overte vašu e-mailovú adresu",
    "Preferred providers" : "Preferovaní poskytovatelia",
    "Preferred Providers" : "Preferovaní poskytovatelia",
    "Allow Nextcloud to request user accounts" : "Povoliť Nextcloudu vyžiadať používateľské kontá",
    "Registration handling app for Nextcloud partners only" : "Aplikácia na spracovanie registrácie iba pre partnerov Nextcloud",
    "Log in" : "Prihlásiť sa",
    "Your account is ready." : "Vaše konto je pripravené.",
    "Set your password" : "Nastavte si heslo",
    "Account name or email" : "Názov účtu alebo e-mail",
    "Password" : "Heslo",
    "Set password" : "Nastaviť heslo",
    "After this step, you will receive a mail to verify your account within 6 hours." : "Po tomto kroku obdržíte do 6 hodín e-mail pre overenie vášho účtu.",
    "Check your email" : "Skontrolujte si e-mail",
    "If a deactivated account for that address exists, we've sent a new verification link. It expires in 6 hours." : "Ak pre túto adresu existuje deaktivované konto, odoslali sme nový overovací odkaz. Platnosť odkazu uplynie o 6 hodín.",
    "Enter your email address" : "Zadajte svoju e-mailovú adresu",
    "Send reactivation email" : "Odoslať e-mail na opätovnú aktiváciu",
    "Preferred providers token" : "Token preferovaných poskytovateľov",
    "WARNING! This token is very important and must be handled carefully. You should only give it to a Nextcloud official!" : "VAROVANIE! Tento token je veľmi dôležitý a treba s ním zaobchádzať opatrne. Poskytnite ho iba oficiálnemu zástupcovi Nextcloudu!",
    "Your provider token" : "Token vášho poskytovateľa",
    "Reset your token" : "Resetovať token",
    "Preferred providers groups" : "Skupiny preferovaných poskytovateľov",
    "Select the groups to which each new account will be added." : "Vyberte skupiny, do ktorých sa pridá každý nový účet.",
    "Select the groups that will be assigned to each unconfirmed account. They will be deleted once the account has been verified." : "Vyberte skupiny, ktoré budú priradené ku každému nepotvrdenému účtu. Po overení účtu budú odstránené.",
    "Select the groups that will be assigned to each confirmed account." : "Vyberte skupiny, ktoré budú priradené ku každému potvrdenému účtu."
},
"nplurals=4; plural=(n % 1 == 0 && n == 1 ? 0 : n % 1 == 0 && n >= 2 && n <= 4 ? 1 : n % 1 != 0 ? 2: 3);");
